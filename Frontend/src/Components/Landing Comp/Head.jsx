    ...
    return (
      <>
        <nav className="fixed z-[100] w-full">
          <div
            className={
              "flex items-center font-popp group bg-white text-slate-200 transition-all ease-in-out duration-300 w-full h-auto " +
              " h-20"
            }
          >
            <a href="/" className="flex items-center text-decoration-none">
              ...
              <div className="w-fit h-auto grid grid-cols-1 gap-y-0 content-center relative group-[.hide]:-mt-1.5 duration-300 ease-out transition-all">
                <span className="font-rome text-[2vmax] text-amber-800 p-0 -mb-[0.75vmax]">
                  VELAMMAL
                </span>
            ...
            <div
              class="items-stretch relative h-max my-auto pb-2 group-[.hide]:-mt-2
                        lg:max-w-[17.5vw] max-w-[40%] ml-2 flex"
            >
              {nacs.map((nac, i) => (
                <div
                  class="duration-200 self-center ease-linear"
                  data-carousel-item
                >
                  <img
                    src={nac}
                    class="block max-h-[5vmax] mt-2 h-full w-auto p-1"
                    alt="naac"
                    key="naac"
                  />
                </div>
            ...
            <div
              className="lg:flex flex-wrap hidden right-0 justify-end grow text-[1.2vmax] max-w-[63.5%] w-fit h-max gap-x-4 gap-y-0
                        duration-300 ease-in-out transition"
            >
              {navs.map((nvt) => (
                <div className="group/nav relative transition-all mt-3">
                  <p
                    className={`align-middle group-[.hide]:top-1 self-center w-fit p-[0.75vmin]
                                        ...
                                        hover:ease-out hover:duration-700 ease-in-out duration-300`}
                  >
                    {nvt.main}
                    <ChevronDownIcon className="size-[1.3vmax] mb-1 ml-1 inline text-black"></ChevronDownIcon>
                  </p>
                  <div
                    className={`grid grid-flow-row content-center rounded-lg outline group-hover/nav:outline-[#fdcc03] outline-transparent 
                                        right-0 top-10 z-[500] absolute group-hover/nav:max-h-[700vh] max-h-0 h-fit ${
                                          nvt.cols > 1
                                            ? "min-w-[55vw]"
                                            : "w-max"
                                        } bg-white outline-offset-2
                    ...
                    style={{
                      gridTemplateColumns: `repeat(${nvt.cols}, minmax(0, 1fr))`,
                    }}
                  >
                    {griddy(nvt.sub, nvt.cod).map((sbj, i, { length }) => (
                      <div className="group/sub relative w-full">
                              ...
                              i === 0 ? "rounded-t-lg" : ""
                                                    ...
                                                    ${
                                                      i === length - 1
                                                        ? "rounded-b-lg"
                                                        : ""
                                                    } bg-gradient-to-l from-[#fdcc03] from-0% via-[#fdcc03] via-50% to-white to-50% border-slate-700
                        ...
                        >
                          <p
                            className={
                              "w-full my-2 align-middle text-nowrap border-slate-500 border-dashed " +
                              (sbj.hrd
                                ? "font-bold border-b-2 text-center"
                                : "text-left")
                            }
                          >
                            {sbj.ttl}
                        ...
                        {sbj.sup.length > 0 ? (
                          <div
                            className="absolute top-0 left-[105%] z-10 group-hover/sub:max-h-[70vh] max-h-0 h-fit overflow-y-hidden bg-white
                          ...
                          >
                            {sbj.sup.map((spj, i, { length }) => (
                              <a
                                className={`no-underline inline-block bg-[length:200%_100%] bg-[position:0%_100%] text-slate-950 -translate-x-[-40vw] px-2
                                                            ${
                                                              i !== length - 1
                                                                ? "border-b"
                                                                : ""
                                                            } bg-gradient-to-l from-[#fdcc03] from-0% via-[#fdcc03] via-50% to-white to-50% 
          ...
          <div
            className="hidden lg:flex pl-4 font-popp bg-[#fdcc03] p-[0.35rem] gap-3 z-10 w-full h-[2.5rem]
          ...
          >
            ...
            <button
              onClick={() => navigate("/naac")}
              className=" mt-1 h-fit md:block hidden"
            >
              {hdrs[3]}
            </button>
            <button
              onClick={() => navigate("/nirf")}
              className=" mt-1 h-fit md:block hidden"
            >
              {hdrs[4]}
            </button>
            ...
            <button className='truncate mt-1 h-fit md:block hidden' onClick={() => navigate('/other-facilities')}>{hdrs[15]}</button>
            <button className='truncate mt-1 h-fit md:block hidden' onClick={() => navigate('/greviences')}>{hdrs[16]}</button>
            <button
              className="truncate mt-1 h-fit md:block hidden"
              onClick={() => navigate("/login")}
            >
              {hdrs[17]}
            </button>
            <p
              className="truncate mt-1 h-fit md:block hidden rounded-full
                        bg-black text-[#fdcc03] px-2"
            >
              {hdrs[18]}
            </p>
            <div
              className={`flex group bg-[#fdcc03] items-center justify-end grow gap-3 ${hdr}`}
            >
              {socls.map((socl, i) => (
                <a href={socl.Link}>
                  <img
