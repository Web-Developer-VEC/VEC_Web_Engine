...
const Research_activities = ({ selectedYear, newAcadamicYear, data, setData }) => {
  // -------------------- State --------------------
  ...
  const defaultState = {
    ...
    research_funding: {
      ...
      ongoing_projects_sanctioned_previous_years: {
        odd_sem: "",
        even_sem: "",
        consolidated: "",
        undertaking: "",
      ...
      sttp_workshops_fdps_entrepreneurship_proposals_and_amount: {
        odd_sem: "",
        even_sem: "",
        consolidated: "",
        undertaking: "",
      ...
      other_student_schemes_iedc_tnscst: {
        odd_sem: "",
        even_sem: "",
        consolidated: "",
        undertaking: "",
    ...
    <>
            ...
            <div key={index} className="form-grid grid-6-upload form-input">
              ...
              >
                <input
                  ...
                  style={{ display: "none" }}
                  onChange={(e) =>
                    handleFileChange("phd_scholars", key, e.target.files[0])
                  }
                />

                {/* Upload / Update Button */}
                <label htmlFor={`upload-phd-${key}`} className="upload-btn">
                  {page2Data.phd_scholars[key].pdf_path ? "Update" : "Upload"}
                </label>

                {/* Eye Icon */}
                {page2Data.phd_scholars[key].pdf_path && (
                  <Eye
                    style={{ cursor: "pointer", width: "18px" }}
                    onClick={() => {
                      const file = page2Data.phd_scholars[key].pdf_path;
      ...
      <div className="section">
        <div className="section-title">
          VII. RESEARCH PUBLICATIONS BY FACULTY
        </div>
        ...
        {[
          ...
          <div key={index} className="form-grid grid-12-upload form-input">
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              {/* Upload / Update Button */}
              <label htmlFor={`upload-research-${key}`} className="upload-btn">
                {page2Data.research_publications[key].pdf_path
                  ? "Update"
                  : "Upload"}
              </label>
              ...
              {page2Data.research_publications[key].pdf_path && (
                <Eye
                  style={{ cursor: "pointer", width: "18px" }}
                  onClick={() => {
                    const file = page2Data.research_publications[key].pdf_path;
                    const fileURL = URL.createObjectURL(file);
                    window.open(fileURL, "_blank");
                  }}
                />
          ...
          <div key={index} className="form-input form-grid grid-6-upload">
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <input
                type="file"
                accept="application/pdf"
                id={`upload-funding-${key}`}
                style={{ display: "none" }}
                onChange={(e) =>
                  handleFileChange("research_funding", key, e.target.files[0])
                }
              />

              {/* Upload / Update Button */}
              <label htmlFor={`upload-funding-${key}`} className="upload-btn">
                {page2Data.research_funding[key].pdf_path ? "Update" : "Upload"}
              </label>

              {/* Eye Icon */}
              {page2Data.research_funding[key].pdf_path && (
                <Eye
                  style={{ cursor: "pointer", width: "18px" }}
                  onClick={() => {
                    const file = page2Data.research_funding[key].pdf_path;
                    const fileURL = URL.createObjectURL(file);
                    window.open(fileURL, "_blank");
                  }}
                />
      ...
      <div className="section">
        ...
        <div className="form-input form-grid grid-6">
          <label className="form-label">
            1. Amount received as Consultancy
          </label>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            {/* Upload / Update Button */}
            <label htmlFor="upload-consultancy" className="upload-btn">
              {page2Data.consultancy.amount_received.pdf_path
                ? "Update"
                : "Upload"}
            </label>
            ...
            {page2Data.consultancy.amount_received.pdf_path && (
              <Eye
                style={{ cursor: "pointer", width: "18px" }}
                onClick={() => {
                  const file = page2Data.consultancy.amount_received.pdf_path;
                  const fileURL = URL.createObjectURL(file);
                  window.open(fileURL, "_blank");
                }}
              />
...
export default Research_activities;
