...
const InCub = ({ toggle, theme }) => {
    ...
    const cmtie = [
        { pos: "President", cls: "basis-2/5" },
        { pos: "Vice-President", cls: "basis-2/5" },
        { pos: 'Convener', cls: "basis-1/4" },
        { pos: 'Industry Expert', cls: "basis-1/4" },
        { pos: 'IPR Expert', cls: "basis-1/4" },
        { pos: 'Alumni Expert', cls: "basis-1/4" },
        { pos: 'Secretary', cls: "basis-1/4" },
        { pos: 'Treasurer', cls: "basis-1/4" },
        { pos: 'Event Coordinator', cls: "basis-1/4" },
        { pos: 'Marketing & Outreach Coordinator', cls: "basis-1/4" },
        { pos: 'Innovation Lab Manager', cls: "basis-1/4" },
        { pos: 'Startup Liaison Officer', cls: "basis-1/4" },
        { pos: 'Industry Relation Manager', cls: "basis-1/4" },
        { pos: 'Research & Development Coordinator', cls: "basis-1/4" },
        { pos: 'Committee Members', cls: "basis-1/4" },
    ];
    const msme = [{
    ...
    return (
        <>
            ...
            <div className="mt-4 mb-24 grid gap-8">
                <div className="flex flex-wrap gap-x-4 gap-y-4 justify-center">
                    {tiles.map((tile, index) => (
                        <div className={`${tile.cls} border-l-8 p-4 border-secd dark:border-drks rounded-xl 
                            bg-gray-200 dark:bg-gray-700`} key={index}>
                ...
                <p className="text-xl text-center font-poppins">Incubation Process</p>
                <div className="flex flex-wrap">
                    <div className="grid grid-cols-1 group gap-6 px-12 lg:px-32 text-center w-fit
                        ...
                        {flow.map((flw, ind) => (
                            ...
                            dark:bg-[color-mix(in_srgb,theme(colors.drks)_10%,black)] relative font-poppins`}
                                 style={{ transitionDelay: `${ind * 100}ms` }} key={ind}>
                ...
                <div className="flex flex-wrap gap-4 justify-center lg:px-0 mt-8">
                    <p className="basis-full text-xl ml-[10vw] font-poppins">Committee</p>
                    {cmtie.map((cmt, i) => (
                        <div className={`${cmt.cls} py-2 px-4 rounded-xl hover:border-l-4 border-secd dark:border-drka
                            ...
                            <p className="text-xl font-poppins">{cmt.pos}</p>
                            {/* Removed cmt.nme reference as it doesn't exist in data */}
                            <p className="text-lg text-accn dark:text-drka">{/* cmt.pos removed as per data */}</p>
                        </div>
                    ))}
                </div>
                <div className="justify-center lg:mx-32 p-4 rounded-xl
                    ...
                    <span className="text-xl font-poppins">MSME (MINISTRY OF MICRO, SMALL & MEDIUM ENTERPRISES)</span>
                    <div className={"h-4"} />
                    {msme.map((msm, i) => (
                        <div className={`${msm.cls}`} key={i}>
                            <p className="text-xl text-accn dark:text-drka font-poppins">{msm.hdr}</p>
                            <p className="text-base font-poppins">{parse(msm.cnt)}</p>
                        </div>
                    ))}
                ...
                <div className="relative overflow-x-auto sm:m-4 lg:mx-32">
                    <p className="text-xl my-4 font-poppins">SEED MONEY RECEIVED FROM OTHER INCUBATION</p>
                                    ...
                                    <tr className="even:bg-[color-mix(in_srgb,theme(colors.secd),transparent_70%)]
                                        ...
                                        {(ind < 1) ?
                                            <th scope="row" rowSpan={cmp.sed.length}
                                                className="px-6 py-4 text-xl font-medium text-gray-900 dark:text-white font-poppins">
                                                {cmp.cpy}
                                            </th> : ""}
                ...
                <div className="lg:mx-32">
                    <p className="text-xl mb-4 text-left font-poppins">Connect with us</p>
                    <p className="py-2 px-8 rounded-xl text-base bg-[color-mix(in_srgb,theme(colors.prim)_85%,black)] w-fit font-poppins">
                        <a className="text-blue-500" href="mailto:Incubation@velammal.edu.in">Incubation@velammal.edu.in</a>
                    </p>
                </div>
            </div>
        </>
