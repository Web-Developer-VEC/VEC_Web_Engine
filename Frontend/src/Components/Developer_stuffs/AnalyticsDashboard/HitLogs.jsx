...
const HitLogs = () => {
  ...
  const filteredData = hitData.filter((item) => {
    ...
    const endpointName = (endpointNames[item.endpoint] || "").toLowerCase();

    return endpoint.includes(query) || endpointName.includes(query);
  });
  ...
  const getWeekValue = (monthName, weekNum, value) => {
    ...
    if (rowMonthIndex === currentMonthIndex && weekNum > currentWeekNumber)
      return "-";
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 flex items-center gap-2 md:gap-3">
              <svg
                className="w-6 h-6 md:w-8 md:h-8 text-[#801828] shrink-0"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-sm md:text-base text-slate-500 mt-1">
              Monitor your endpoint traffic and usage statistics
            </p>
          </div>
          ...
          <div className="relative w-full md:w-96 md:shrink-0 group/search">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <svg
                className="h-5 w-5 text-slate-400 transition-colors group-focus-within/search:text-[#801828]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              >
                ...
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-3 transition-colors group-hover:bg-[#801828]/[0.04]">
                  <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <svg
                      className="w-5 h-5 text-slate-400 shrink-0 transition-all duration-200 group-hover:text-[#801828] group-hover:rotate-12"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <span className="transition-colors group-hover:text-[#801828]">
                      ...
                      <tbody className="bg-white divide-y divide-slate-100">
                        {Object.entries(item.thisYear.monthly).map(
                          ([month, stats]) => (
                            <tr
                              key={month}
                              className="hover:bg-[#801828]/[0.04] transition-colors"
                            >
                              <td className="px-4 py-2 text-sm font-medium text-slate-800 whitespace-nowrap">
                                {month}
                              </td>
                              <td className="px-4 py-2 text-sm text-[#801828] font-bold">
                                {stats.overall_month_count ?? 0}
                              </td>
                              <td className="px-4 py-2 text-sm text-slate-600">
                                {getWeekValue(month, 1, stats.week1)}
                              </td>
                              <td className="px-4 py-2 text-sm text-slate-600">
                                {getWeekValue(month, 2, stats.week2)}
                              </td>
                              <td className="px-4 py-2 text-sm text-slate-600">
                                {getWeekValue(month, 3, stats.week3)}
                              </td>
                              <td className="px-4 py-2 text-sm text-slate-600">
                                {getWeekValue(month, 4, stats.week4)}
                              </td>
                            </tr>
                          ),
                        )}
...
const StatCard = ({ label, value, icon, isHighlight = false }) => (
  <div className="group/stat flex flex-col rounded-lg px-2 py-1.5 -mx-2 -my-1.5 transition-all duration-150 hover:bg-slate-50 hover:-translate-y-0.5 cursor-default">
    <span className="flex items-center gap-1.5 text-xs text-slate-500 uppercase tracking-wider font-medium mb-1 transition-colors group-hover/stat:text-[#801828]">
      <span className="transition-transform duration-150 group-hover/stat:scale-110">
        {icon}
      </span>{" "}
      {label}
    </span>
