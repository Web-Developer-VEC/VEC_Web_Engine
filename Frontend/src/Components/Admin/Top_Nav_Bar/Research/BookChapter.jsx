...
export default function AdminBookChapter({ theme, toggle }) {
  ...
  const handleDeleteButton = (index) => {
    setBookChapter((prev) => prev.filter((_, i) => i !== index));
  };
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[2147483647]">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg w-96">
              ...
              <button
                ...
                onClick={() => {
                  if(!newPdf || !newYear){
                    alert("Submit with all the field")
                  }
                  setBookChapter((prev) => [
                    ...prev,
                    { year: newYear, pdf_path: newPdf },
                  ]);
                  setShowPopup(false);
                  setNewYear("");
                  setNewPdf("");
