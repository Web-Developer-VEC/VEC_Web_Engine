...
const revertChange = (itemId, action, isFaculty = false) => {
  ...
  if (isFaculty) {
    // Restore committed faculty
    setPendingFaculty(deepCopy(committedFaculty));
    setFaculty(deepCopy(committedFaculty));
  } else {
    if (action === "Deleted") {
      // Restore the deleted student from committedStudents
  ...
  // After reverting, recompute changes and clear draft if nothing remains
...
// Fix for image paths: strip BASE_URL if present
const normalizeImage = (path) => {
  if (!path) return "";
  return path.replace(/^https?:\/\/[^/]+/, ""); // remove domain part
};
...
const getChanges = () => {
  ...
  // ---- Student changes (use stable string keys) ----
  // Deleted: in committed but not current
...
(
  ...
  <input
    ...
    value={faculty?.name || ""}
    onChange={(e) =>
      handleChangeFaculty("name", e.target.value.toUpperCase())
    }
  ...
  onChange={(e) =>
    handleChangeFaculty( "designation", toTitleCase(e.target.value))
  }
...
<input
  ...
  onChange={(e) =>
    handleChangeStudent(index, "designation", toTitleCase(e.target.value))
  }
  className="border p-1 w-full"
  placeholder="Designation"
/>
  ...
  {changes.map((ch, i) => (
    <tr key={i}>
      ...
      <td className="border p-2">
        {/* Show revert for Edited, Added, and Deleted */}
<button
