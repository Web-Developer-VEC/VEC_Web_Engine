  ...
  setShowDeleteModal(false);
    ...
    const getChanges = () => {
        ...
        for (const [id, newItem] of pendingMap.entries()) {
            const oldItem = originalMap.get(id);
            if (!oldItem) {
                changes.push({ action: "Added", section: "Certificate Details", changes: newItem.name, rowId: id });
            } else if (oldItem.name !== newItem.name || oldItem.pdf_path !== newItem.pdf_path) {
                changes.push({ action: "Edited", section: "Certificate Details", changes: newItem.name, rowId: id });
            }
        }
        for (const [id, oldItem] of originalMap.entries()) {
            if (!pendingMap.has(id)) {
                changes.push({ action: "Deleted", section: "Certificate Details", changes: oldItem.name, rowId: id });
            }
        }
        return changes;
    };
                ...
                <div className="fixed inset-0 bg-text/50 flex items-center justify-center z-50">
                    <div className="bg-prim p-6 rounded-lg shadow-lg border w-[90%] max-w-md">
                        ...
                        <p className="text-gray-600 mb-6">
                            Are you sure you want to delete {selectedRows.size} selected certificate
