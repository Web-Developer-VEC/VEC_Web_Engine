    ...
    {!isEditing && (
      ...
      >
        ...
        <div className="iic-members-grid">
          {rows.map((faculty, i) => (
            <div key={faculty.id || i} className={`faculty-card dark:bg-text relative ${faculty.selected ? "ring-2 ring-blue-500" : ""}`}>
              {isEditing && (
                <input
                  type="checkbox"
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const formattedData = data.map((categoryBlock, idx) => ({
        ...
        members: categoryBlock.members?.map((member, memIdx) => ({
