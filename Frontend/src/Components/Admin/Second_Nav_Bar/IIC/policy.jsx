...
export default function IicFacPolicy({ data }) {
    ...
    // Load initial data and format it correctly
    useEffect(() => {
    ...
    const getChanges = () => {
        ...
        for (const [id, newItem] of pendingMap.entries()) {
            const oldItem = originalMap.get(id);
            if (!oldItem) {
                changes.push({ action: "Added", section: "Policy Details", changes: newItem.name, rowId: id });
            } else if (oldItem.name !== newItem.name || oldItem.pdf_path !== newItem.pdf_path) {
                changes.push({ action: "Edited", section: "Policy Details", changes: newItem.name, rowId: id });
            }
        }
        for (const [id, oldItem] of originalMap.entries()) {
            if (!pendingMap.has(id)) {
                changes.push({ action: "Deleted", section: "Policy Details", changes: oldItem.name, rowId: id });
            }
        }
        return changes;
    };
    ...
    return (
        <div className="p-6 mt-4 pb-20">
            ...
            <div className="nirf-details dark:bg-[color-mix(in_srgb,theme(colors.drkp)_95%,prim)] height">
                ...
                {/* Show PDF */}
                {activePdf && (
                    <div className="relative border p-8 mt-10 w-[94%] mx-auto bg-prim dark:bg-drkp shadow-lg rounded-lg">
                            ...
                            {isEditing ? (
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (index !== -1) handleChange(index, "name", e.target.value);
                                    }}
                ...
                <div className="fixed inset-0 bg-text/50 flex items-center justify-center z-50">
                    <div className="bg-prim p-6 rounded-lg shadow-lg border w-[90%] max-w-md">
                        ...
                        <p className="text-gray-600 mb-6">
                            Are you sure you want to delete {selectedRows.size} selected policy
