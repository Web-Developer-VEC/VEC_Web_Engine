...
const AdminIQAC = ({ toggle, theme }) => {
  ...
  function IqaObj() {
    ...
    const handleSave = () => {
      ...
      const aboutChange =
        editedData.about !== iqacData.about
          ? {
              old: iqacData.about,
      ...
      newObjectives.forEach((newObj, newIndex) => {
        ...
        for (let oldIndex = 0; oldIndex < oldObjectives.length; oldIndex++) {
          if (
            !matchedOldIndices.has(oldIndex) &&
            oldObjectives[oldIndex] === newObj
          ) {
    ...
    const handleUndoObjective = (changeIndex) => {
      ...
      const remainingChanges = changes.objectives.filter(
        (_, idx) => idx !== changeIndex,
      );
      ...
      remainingChanges.forEach((change) => {
        if (change.action === "add") {
          ...
          const deleteIndex = reconstructedObjectives.findIndex(
            (obj) => obj === change.old,
          );
          if (deleteIndex !== -1) {
          ...
          const editIndex = reconstructedObjectives.findIndex(
            (obj) => obj === change.old,
          );
          if (editIndex !== -1) {
      ...
      setSavedData((prev) => ({
        ...prev,
        objectives: reconstructedObjectives,
      }));
      setEditedData((prev) => ({
        ...prev,
        objectives: reconstructedObjectives,
                  ...
                  <tbody>
                        ...
                        <td className="py-3 px-2">
                          <div className="text-sm">
                            <span className="line-through text-gray-500 block">
                              {changes.about.old.substring(0, 50)}...
                            </span>
                            <span className="text-green-600 block">
                              {changes.about.new.substring(0, 50)}...
                            </span>
                          </div>
                        </td>
                    ...
                    {changes.objectives.map((change, idx) => {
                      ...
                      return (
                        <tr
                          key={`objective-${idx}`}
                          className={`border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 ${actionDisplay.bgColor} bg-opacity-30`}
                        >
                          <td className="py-3 px-2">
                            <span
                              className={`${actionDisplay.color} font-medium`}
                            >
                              {actionDisplay.text}
                            </span>
                          </td>
                              ...
                              {change.action === "edit" && (
                                <>
                                  <span className="text-gray-500 line-through block text-xs">
                                    Old: "{change.old.substring(0, 30)}..."
                                  </span>
                                  <span className="text-green-600 block text-sm">
                                    New: "{change.new.substring(0, 30)}..."
                                  </span>
                                </>
                              )}
                            </div>
                          ...
                          <div className="text-sm">
                            {field === "image_path" ? (
                              <span className="text-green-600">
                                New image: {changes[field].new}
                              </span>
                            ) : (
                              <>
                                <span className="line-through text-gray-500 block text-xs">
...
console.log(iqacData);
                  ...
                  {!uploadedFile ? (
                    <>
                      ...
                      <button
                        ...
                        className="bg-gray-500 text-white px-4 py-2 rounded-[10px] hover:bg-gray-600"
                      >
                        Cancel
                      </button>
                    </>
                    ...
                    <>
                      ...
                      <button
                        onClick={() => setSavedFile(true)}
                        className="bg-secd text-text px-4 py-2 rounded-[10px] hover:bg-[#800000] hover:text-drkt"
                      >
                        Save
                      </button>
                    </>
  ...
  className="bg-gray-500 text-white px-4 py-2 rounded-[10px] hover:bg-gray-600"
                      ...
                      <button
                        onClick={() => setShowRequestModal(true)}
                        className="bg-secd text-text px-4 py-2 rounded-[10px] hover:bg-[#800000] hover:text-drkt flex items-center gap-2"
                      >
                        <Send size={18} />
                        Request
                      </button>
              ...
              <embed
                ...
                src={
                  uploadedFile?.fileURL
                    ? uploadedFile.fileURL + "#toolbar=0"
                    : UrlParser(
                        Array.isArray(iqacData) && iqacData[0]?.pdf_path,
                      ) + "#toolbar=0"
                }
