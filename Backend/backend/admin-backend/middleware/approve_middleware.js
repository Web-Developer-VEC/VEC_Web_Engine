...
async function getTempRequests(req, res) {
  try {
    ...
    for (const collectionName of req_collection) {
      ...
      // let admindetails = pendingRequests.admin;
      // group by action
...
async function handleTempApproval(req, res, next) {
  try {
    ...
    // Fetch the pending doc (you can filter by any condition, here we use first pending)
    const tempDoc = await tempCollection.findOne({ status: "pending" });
    if (!tempDoc) {
    ...
    if (status === "approved") {
      ...
      // ✅ Attach to req and go to next handler
      const updatedDoc = await tempCollection.findOne({ _id: tempDoc._id });

      req.tempDoc = updatedDoc;
      const DB = getDb();
