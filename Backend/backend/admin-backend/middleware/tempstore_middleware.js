...
module.exports = async function storeTempMiddleware(req, res, next) {
  try {
    ...
    const tempDoc = {
      ...
      meta_data: meta_data || filePaths? {...(meta_data || {}), filePaths: filePaths || null,}: null,
      original_data: original_data || null,
      admin: { id: req.session.admin.id, name: req.session.admin.name, role: req.session.admin.role },
      status: "pending",
      createdAt: new Date(),
    };
