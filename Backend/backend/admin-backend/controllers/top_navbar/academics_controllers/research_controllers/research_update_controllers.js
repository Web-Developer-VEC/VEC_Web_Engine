async function updateData(tempDoc, mainCollection) {
  ...
  if (collection_type === "research") {
    ...
    for (let i = 0; i < meta_data.research.length; i++) {
      const newItem = meta_data.research[i];
      const originalItem = original_data.research[i];
      ...
      await mainCollection.updateOne(
        ...
        {
          $set: {
            "data.$[d].content.$[c].research.$[r]": newItem
          }
        },
        {
          arrayFilters: [
    ...
    return {success:true,  message: `Research data updated successfully for category ${category} and year ${meta_data.year}` };
  }
}
