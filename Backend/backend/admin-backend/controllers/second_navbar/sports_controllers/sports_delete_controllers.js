  ...
  try {
    ...
    if (singleDocTypes.includes(collection_type)) {
      ...
      if (Array.isArray(meta_data)) {
        valueToDelete = meta_data[0]; // delete first item
      } else if (typeof meta_data === "object" && meta_data !== null) {
        valueToDelete = Object.values(meta_data)[0];
      } else {
        valueToDelete = meta_data;
      }

      await mainCollection.updateOne(
        { type: collection_type },
        { $pull: { data: valueToDelete } }
      );

      return {
        success: true,
        message: `Deleted successfully from ${collection_type}`,
        deleted: valueToDelete,
      };
    }

    // 2️⃣ Multi-doc flat objects
