  ...
  try {
    ...
    if (collection_type === "general_info") {
      ...
      if (category === "Menu") {
        ...
        const menuContainer =
          doc.data[catIndex].content?.[0]?.hostel_menu?.[0];
        ...
        Object.keys(meta_data).forEach((key) => {
          if (menuContainer[key]) {
            menuContainer[key] = meta_data[key];
          }
        });
      ...
      if (!original_data?.section) {
        throw new Error("Section required to update content");
      }
      ...
      if (contentIndex === -1) {
        throw new Error(`Content not found for section: ${original_data.section}`);
      }
