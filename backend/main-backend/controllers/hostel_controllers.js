...
async function getHostelDetails(req, res) {
  try {
    ...
    if (!ALLOWED_HOSTEL_TYPES.includes(type)) {
      return res.status(400).json({ error: `"${type}" is not a valid IQAC section` });
    }

    const db = getDb();
    const collection = db.collection('hostel_details');
    ...
    console.error('Error fetching hostel section:', error);
    await logError(req, error, 'Error fetching hostel section', 500);
