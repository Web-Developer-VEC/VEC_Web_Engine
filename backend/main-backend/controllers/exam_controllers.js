...
async function getExamsSection(req, res) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(type)) {
      return res.status(400).json({ error: `"${type}" is not a valid exams section` });
    }
    ...
    console.error('Error fetching exams section:', error);
    await logError(req, error, 'Error fetching exams section', 500);
    return res.status(500).json({ error: 'Internal Server Error' });
  }
}

module.exports = { getExamsSection };
